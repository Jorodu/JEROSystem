			...
			<div class="d-flex">				
                                <div class="col-lg-12 about-right slider-right-con">
                                    <div style="text-align: left;">                                        
                                        <b style="font-size: 42px;color: #5DADF7;">Aportes</b>

                                        <c:if test="${lstDatos.size() == 0}">
                                            No hay productos publicados.
                                        </c:if>   
                                        <div class="header-contact-w3ls text-center mt-lg-0 mt-4" style="text-align: center;">
                                            <a href="<%=request.getContextPath()%>/crearAporte?id=0" class="rounded py-sm-3 py-2 px-5" style="background-color: #5DADF7;">
                                            ...
                                            <tbody>
                                                <c:forEach var="dato" items="${lstDatos}">      
                                                    <tr>
                                                        <td>
                                                            <c:if test="${perfil == 1}">
                                                                <a href="<%=request.getContextPath()%>/crearAporte?id=${dato.idAporte}">Editar</a>
                                                            </c:if>
                                                        </td>
                                                        <td>${dato.nombre_folder}</td>
                                                        <td style="text-align: right;"><fmt:formatNumber type = "number" maxFractionDigits = "3" value = "${dato.valor_aporte}" />
                                                        </td>
                                                        <td><fmt:formatDate pattern="dd/MM/yyyy" value="${dato.fecha_registro_aporte}" /></td>
                                                        <td><a href="<%=request.getContextPath()%>/resources/imagenes/${dato.soporte_aporte}" target="_blank">Ver</a></td>
