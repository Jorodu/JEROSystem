			...
			<div class="d-flex">				
                                <div class="col-lg-12 about-right slider-right-con">
                                    <div style="text-align: left;">
                                        <b style="font-size: 42px;color: #5DADF7;">Folders</b>
                                    
                                        <c:if test="${lstDatos.size() == 0}">
                                            No hay productos publicados.
                                        </c:if>       
                                        <div class="header-contact-w3ls text-center mt-lg-0 mt-4" style="text-align: center;">
                                            <a href="<%=request.getContextPath()%>/crearFolder?id=0" class="rounded py-sm-3 py-2 px-5" style="background-color: #5DADF7;">
                                                <i class="fas fa-arrow-up"></i> Adicionar Carpeta
                                            </a>
                                        </div>
                                            ...
                                            <tbody>
                                                <c:forEach var="dato" items="${lstDatos}">      
                                                    <tr>
                                                        <td>
                                                            <c:if test="${perfil == 1}">
                                                                <a href="<%=request.getContextPath()%>/crearFolder?id=${dato.idFolder}">Editar</a>
                                                            </c:if>
                                                                <br/>
                                                                <a href="<%=request.getContextPath()%>/crearAfiliadoPorCarpeta?id=${dato.idFolder}">Asociar Afiliados</a>

                                                        </td>
                                                        <td>FUN-${dato.idFolder}</td>                                                    
